...
export default function Products({ ctx }) {
  ...
  const filtered = useMemo(() => products.filter(p => {
    ...
    return p.name.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q) || (p.alias || "").toLowerCase().includes(q);
  }), [products, search, cf]);
  ...
  const doSave = () => {
    ...
    const d = { ...form, margin };
    if (editId) { saveProducts(products.map(p => p.id === editId ? d : p)); addLog("updated", "product", form.name); }
    else { saveProducts([...products, { ...d, id: uid() }]); addLog("created", "product", form.name); }
    setModal(false);
  };
  ...
  const doSubmit = () => {
    if (!form.name || !form.sku) return;
    const margin = calcMgn(form.mrp, form.purchasePrice);
    addChangeReq({ entity: "product", action: editId ? "update" : "create", entityId: editId || null, entityName: form.name, currentData: editId ? products.find(p => p.id === editId) : null, proposedData: { ...form, margin } });
    setModal(false);
  };
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    <div style={{ display: "flex", gap: 6 }}>
      {["products", "categories"].map(t => <button key={t} onClick={() => setTab(t)} style={{ padding: "8px 18px", borderRadius: 10, border: `1px solid ${tab === t ? T.accent : T.borderSubtle}`, cursor: "pointer", fontWeight: 600, fontSize: 13, background: tab === t ? T.accent : "transparent", color: tab === t ? "#fff" : T.textSub, transition: "all .15s" }}>{t.charAt(0).toUpperCase() + t.slice(1)}</button>)}
    </div>
    ...
    {tab === "products" && <>
      ...
      <div className="filter-wrap">
        <div style={{ position: "relative", flex: "1 1 200px" }}>
          ...
          <input className="inp" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, SKU, alias…" style={{ paddingLeft: 34 }} />
        </div>
        <GS value={cf} onChange={e => setCf(e.target.value)} placeholder="All Categories">{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</GS>
        <GBtn onClick={() => { setForm({ unit: "pcs", minStock: 10, gstRate: "0" }); setEditId(null); setModal(true); }} icon={<Plus size={14} />}>Add Product</GBtn>
      </div>
      <div className="glass" style={{ borderRadius: T.radius, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead><tr style={{ background: T.isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.025)" }}>{["Product", "SKU", "Category", "MRP", "Cost", "Margin", "Stock", "Status", ""].map(h => <th key={h} className="th" style={{ textAlign: ["MRP", "Cost", "Margin", "Stock"].includes(h) ? "right" : "left" }}>{h.toUpperCase()}</th>)}</tr></thead>
          <tbody>{filtered.slice((pg - 1) * ps, pg * ps).map(p => {
            const stock = getStock(p.id);
            const cat = categories.find(c => c.id === p.categoryId);
            return <tr key={p.id} className="trow">
              <td className="td"><div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <td className="td r" style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, color: T.accent, fontSize: 13 }}>₹{Number(p.mrp || 0).toLocaleString("en-IN")}</td>
              <td className="td r m">₹{Number(p.purchasePrice || 0).toLocaleString("en-IN")}</td>
              <td className="td r" style={{ color: T.green, fontWeight: 600 }}>{p.margin}%</td>
              <td className="td r" style={{ fontWeight: 700, color: stock <= 0 ? T.red : stock <= p.minStock ? T.amber : T.text }}>{stock}</td>
              <td className="td"><StChip stock={stock} min={Number(p.minStock)} /></td>
              <td className="td"><div style={{ display: "flex", gap: 5 }}>
                ...
                {isAdmin && <button className="btn-danger" onClick={() => { if (window.confirm("Delete?")) saveProducts(products.filter(x => x.id !== p.id)); }} style={{ padding: "4px 7px" }}><Trash2 size={12} /></button>}
              </div></td>
            </tr>;
          })}</tbody>
        </table>
    ...
    {tab === "categories" && <>
      <div style={{ display: "flex", justifyContent: "flex-end" }}><GBtn onClick={() => { setCatForm({ name: "", color: "#C05C1E" }); setCatEdit(null); setCatModal(true); }} icon={<Plus size={14} />}>Add Category</GBtn></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 14 }}>
        {categories.map(c => {
          const cnt = products.filter(p => p.categoryId === c.id).length;
          return <div key={c.id} className="glass" style={{ padding: 18, borderRadius: T.radius }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, color: T.text, fontSize: 14 }}>{c.name}</div><div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>{cnt} product{cnt !== 1 ? "s" : ""}</div></div>
              <div style={{ display: "flex", gap: 5 }}>
                <button className="btn-ghost" onClick={() => { setCatForm(c); setCatEdit(c.id); setCatModal(true); }} style={{ padding: "4px 7px" }}><Edit2 size={12} /></button>
    ...
    <Modal open={modal} onClose={() => setModal(false)} title={editId ? "Edit Product" : "Add Product"} width={540} footer={footer}>
      <div className="fgrid">
        ...
        <Field label="Category"><GS value={form.categoryId || ""} onChange={e => ff("categoryId", e.target.value)} placeholder="Select">{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</GS></Field>
    ...
    <Modal open={catModal} onClose={() => setCatModal(false)} title={catEdit ? "Edit Category" : "Add Category"} width={340}
      footer={<><GBtn v="ghost" onClick={() => setCatModal(false)}>Cancel</GBtn><GBtn onClick={saveCat}>{catEdit ? "Save" : "Add"}</GBtn></>}>
