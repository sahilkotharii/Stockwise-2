...
import { LayoutDashboard, TrendingUp, ShoppingCart, Package, Truck, ArrowLeftRight, BarChart2, Settings, LogOut, Layers, ChevronRight, RefreshCw, Bell, CheckCircle, AlertTriangle, Sun, Moon, Box, RotateCcw, PieChart } from "lucide-react";
...
export const ALL_NAV = [
  ...
  { id: "pnl", label: "P&L", icon: PieChart, adminOnly: true },
...
export function Sidebar({ page, setPage, user, onLogout, col, setCol, syncSt, lastSync, onSync, toggleTheme, isDark, pendingCnt }) {
  ...
  return <div className="desktop-sidebar glass" style={{ position: "fixed", left: 12, top: 12, bottom: 12, width: col ? T.sidebarC : T.sidebarW, borderRadius: 20, display: "flex", flexDirection: "column", zIndex: 50, transition: "width .2s", overflow: "hidden" }}>
    <div style={{ padding: col ? "14px 10px" : "18px 16px", display: "flex", alignItems: "center", gap: 10, borderBottom: `1px solid ${T.borderSubtle}`, minHeight: 70 }}>
      ...
      {!col && <div><div style={{ fontFamily: T.displayFont, fontWeight: 800, fontSize: 15, color: T.text, letterSpacing: "-0.02em" }}>StockWise</div><div style={{ fontSize:11, color: T.textMuted, fontWeight: 500, marginTop: 1 }}>Pipal Home</div></div>}
    </div>
    <nav style={{ flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", gap: 2, overflowY: "auto" }}>
      {items.map(n => <button key={n.id} onClick={() => setPage(n.id)} className={`nav-item${page === n.id ? " active" : ""}`} title={col ? n.label : ""} style={{ justifyContent: col ? "center" : "flex-start", padding: col ? "10px" : "9px 12px", position: "relative" }}>
        ...
        {n.id === "approvals" && pendingCnt > 0 && <span style={{ position: "absolute", top: 6, right: col ? 4 : 8, minWidth: 18, height: 18, borderRadius: 99, background: T.red, color: "#fff", fontSize:11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>{pendingCnt}</span>}
      </button>)}
    </nav>
    <div style={{ padding: "10px 8px", borderTop: `1px solid ${T.borderSubtle}` }}>
      {!col && <div style={{ padding: "10px 12px", borderRadius: 12, background: T.isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", marginBottom: 8 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 }}><span style={{ fontSize:11, fontWeight: 700, color: T.textMuted, letterSpacing: "0.05em" }}>SYNC</span><button onClick={onSync} className="btn-ghost" style={{ padding: "3px 7px", fontSize: 11, borderRadius: 6 }}><RefreshCw size={11} style={{ animation: syncSt === "syncing" ? "spin 1s linear infinite" : "none" }} /></button></div>
        <span className="badge" style={{ background: syncSt === "success" ? T.greenBg : syncSt === "syncing" ? T.blueBg : `${T.accent}12`, color: syncSt === "success" ? T.green : syncSt === "syncing" ? T.blue : T.textMuted, fontSize:11 }}>{syncSt === "syncing" ? "Syncing…" : syncSt === "success" ? `Synced ${lastSync || ""}` : "Offline"}</span>
      </div>}
...
export function MobNav({ page, setPage, user, pendingCnt }) {
  const T = useT();
  const items = visNav(user);
  return <div className="mobile-nav">{items.map(n => <button key={n.id} onClick={() => setPage(n.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "5px 10px", border: "none", background: "transparent", cursor: "pointer", color: page === n.id ? T.accent : T.textMuted, fontSize:11, fontWeight: 600, flexShrink: 0, minWidth: 48, position: "relative" }}><n.icon size={18} /><span style={{ whiteSpace: "nowrap" }}>{n.label.split(" ")[0]}</span>{n.id === "approvals" && pendingCnt > 0 && <span style={{ position: "absolute", top: 2, right: 6, minWidth: 15, height: 15, borderRadius: 99, background: T.red, color: "#fff", fontSize:11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 2px" }}>{pendingCnt}</span>}</button>)}</div>;
}
  ...
  return <div style={{ position: "sticky", top: 12, zIndex: 40, marginBottom: 20 }}>
    <div className="glass" style={{ borderRadius: 14, padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span className="badge" style={{ background: syncSt === "success" ? T.greenBg : syncSt === "syncing" ? T.blueBg : `${T.accent}12`, color: syncSt === "success" ? T.green : syncSt === "syncing" ? T.blue : T.textMuted, cursor: "pointer", fontSize:11 }} onClick={onSync}><RefreshCw size={10} style={{ animation: syncSt === "syncing" ? "spin 1s linear infinite" : "none" }} />{syncSt === "syncing" ? "Syncing" : syncSt === "success" ? `Synced ${lastSync || ""}` : "Offline"}</span>
        <div style={{ position: "relative", cursor: "pointer" }}>
          <div onClick={() => setShowNotifs(!showNotifs)}>
            ...
            {badgeCnt > 0 && <div style={{ position: "absolute", top: -4, right: -4, minWidth: 17, height: 17, borderRadius: 99, background: T.red, color: "#fff", fontSize:11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 3px" }}>{badgeCnt}</div>}
          </div>
          {showNotifs && (
