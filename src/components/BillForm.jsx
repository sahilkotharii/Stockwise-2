...
export default function BillForm({ type, bills, onSave, products, vendors, channels, getStock }) {
  ...
  const [items, setItems] = useState([{ id: uid(), productId: "", qty: 1, price: "", gstRate: 0, isDamaged: false }]);
  ...
  // Purchase: GST is added on top of purchase price. Sale: MRP already includes GST (shown as info only).
  const totalGst = valid.reduce((s, i) => s + Number(i.qty) * Number(i.price) * Number(i.gstRate || 0) / 100, 0);
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      items: valid.map(i => ({
        ...
        gstAmount: Number(i.qty) * Number(i.price) * Number(i.gstRate || 0) / 100,
        isDamaged: Boolean(i.isDamaged),
        mrp: products.find(p => p.id === i.productId)?.mrp || Number(i.price)
      })),
      subtotal, totalGst: type === "purchase" ? totalGst : 0,
  ...
  return <form id={type + "-form"} onSubmit={e => { e.preventDefault(); handleSave(); }} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 12, overflow: "hidden" }}>
        ...
        {items.map((item, i) => {
          ...
          return <div key={item.id} className="bill-item-row">
            ...
            <div>
              <GIn type="number" min="0" step="0.01" value={item.price} onChange={e => upItem(item.id, "price", e.target.value)} placeholder={type === "sale" ? "MRP (incl. GST)" : "Cost (ex-GST)"} />
              {gstRate > 0 && lineBase > 0 && (
                <div style={{ fontSize: 10, marginTop: 3, color: type === "purchase" ? T.amber : T.textMuted, fontWeight: 600 }}>
                  {type === "purchase"
                    ? `+ GST @${gstRate}%: ${fmtCur(lineGst)}`
                    : `GST incl. @${gstRate}%: ${fmtCur(lineGst)}`}
                </div>
              )}
            </div>
            <div>
              ...
              {type === "purchase" && gstRate > 0 && lineBase > 0 && (
                <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2 }}>base {fmtCur(lineBase)}</div>
              )}
            </div>
    ...
    <div className="fgrid">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, justifyContent: "flex-end" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textSub }}><span>Subtotal {type === "purchase" ? "(ex-GST)" : ""}</span><span style={{ fontWeight: 600 }}>{fmtCur(subtotal)}</span></div>
        {type === "purchase" && totalGst > 0 && (
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.amber }}>
            <span>GST (as per product rates)</span><span style={{ fontWeight: 600 }}>+{fmtCur(totalGst)}</span>
          </div>
        )}
        {type === "sale" && totalGst > 0 && (
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textMuted }}>
            <span>GST incl. in MRP</span><span>{fmtCur(totalGst)}</span>
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700, color: T.text, borderTop: `1px solid ${T.borderSubtle}`, paddingTop: 8 }}>
          <span>Total {type === "purchase" ? "(incl. GST)" : "(GST incl.)"}</span>
