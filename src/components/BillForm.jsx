...
function ProductSearch({ value, onChange, products, placeholder }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 200, marginTop: 3, background: T.surfaceStrong, border: `1px solid ${T.borderSubtle}`, borderRadius: 10, boxShadow: T.shadowLg, maxHeight: 220, overflowY: "auto" }}>
          {filtered.length === 0
            ...
            : filtered.map(p => (
              ...
              >
                ...
                <div style={{ fontSize: 10, color: T.textMuted, marginTop: 1 }}>{p.sku}{p.gstRate > 0 ? ` · GST ${p.gstRate}%` : ""} · ₹{Number(p.mrp || 0).toLocaleString("en-IN")}</div>
              </div>
            ))
          }
        </div>
...
export default function BillForm({ type, bills, onSave, products, vendors, channels, getStock, existingBill }) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      discType, discValue: Number(discount || 0), discAmount: discAmt, total, notes,
      ...
      <div>
        ...
        <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 12, overflow: "visible" }}>
          <div className="bill-item-hdr">
            {["Product", "Qty", type === "sale" ? "MRP (GST incl.)" : "Cost (ex-GST)", "Total", "Dmg", ""].map((h, i) => (
          ...
          {items.map((item, i) => {
            ...
            // GST fix applied here too
            const lineGst = type === "sale"
              ? lineBase * gstRate / (100 + gstRate)
              : lineBase * gstRate / 100;
            const lineTotal = type === "purchase" ? lineBase + lineGst : lineBase;

            return (
              <div key={item.id} className="bill-item-row" style={{ overflow: "visible" }}>
                <div style={{ overflow: "visible" }}>
                  <ProductSearch
                    value={item.productId}
                    onChange={v => upItem(item.id, "productId", v)}
                    products={products}
                ...
                <div>
                  ...
                  {gstRate > 0 && lineBase > 0 && (
                    <div style={{ fontSize: 10, marginTop: 2, color: type === "purchase" ? T.amber : T.textMuted }}>
                      {type === "purchase" ? `+GST: ${fmtCur(lineGst)}` : `GST: ${fmtCur(lineGst)}`}
                    </div>
                  )}
                </div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text }}>{fmtCur(lineTotal)}</div>
                  {type === "purchase" && gstRate > 0 && lineBase > 0 && (
                    <div style={{ fontSize: 10, color: T.textMuted }}>base {fmtCur(lineBase)}</div>
                  )}
                </div>
                <div style={{ display: "flex", justifyContent: "center" }}>
                  <input type="checkbox" className="cb" checked={item.isDamaged} onChange={e => upItem(item.id, "isDamaged", e.target.checked)} />
                </div>
                <button type="button" onClick={() => remItem(item.id)} className="btn-danger" style={{ padding: "4px", opacity: items.length <= 1 ? .3 : 1 }} disabled={items.length <= 1}>
                  <X size={12} />
                </button>
              </div>
            );
      ...
      <div className="fgrid">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, justifyContent: "flex-end" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textSub }}>
            <span>Subtotal{type === "purchase" ? " (ex-GST)" : ""}</span>
          ...
          {type === "sale" && totalGst > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textMuted }}>
              <span>GST incl. in above</span><span>{fmtCur(totalGst)}</span>
            </div>
          )}
          {discAmt > 0 && (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700, color: T.text, borderTop: `1px solid ${T.borderSubtle}`, paddingTop: 8 }}>
            <span>Total{type === "purchase" ? " (incl. GST)" : ""}</span>
