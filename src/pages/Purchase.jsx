  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    <div className="glass" style={{ padding: 18, borderRadius: T.radius }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        ...
        <GBtn sz="sm" onClick={() => setModal(true)} icon={<Plus size={14} />}>New Purchase Bill</GBtn>
      </div>
      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        ...
        <tbody>{purBills.slice((pg - 1) * ps, pg * ps).map(b => {
          ...
          return <React.Fragment key={b.id}>
            ...
            {exp[b.id] && <tr style={{ background: T.isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.015)" }}><td colSpan={7} style={{ padding: "12px 20px", borderBottom: `1px solid ${T.borderSubtle}` }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {(b.items || []).map((it, idx) => <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.text }}>
                  <span>{it.productName} × {it.qty} {it.isDamaged ? <span style={{ color: T.red, fontWeight: 600, fontSize: 10, marginLeft: 6 }}>DAMAGED / REJECTED</span> : ""}</span>
