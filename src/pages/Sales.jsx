      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        ...
        <tbody>{saleBills.slice((pg - 1) * ps, pg * ps).map(b => {
          ...
          return <React.Fragment key={b.id}>
            ...
            {exp[b.id] && <tr style={{ background: T.isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.015)" }}><td colSpan={9} style={{ padding: "12px 20px", borderBottom: `1px solid ${T.borderSubtle}` }}>
              <div style={{ fontWeight: 700, color: T.textMuted, fontSize: 10, marginBottom: 8 }}>BILL ITEMS</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {(b.items || []).map((it, idx) => <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.text }}>
                  <span>{it.productName} × {it.qty} {it.isDamaged ? <span style={{ color: T.red, fontWeight: 600, fontSize: 10, marginLeft: 6 }}>DAMAGED</span> : ""}</span>
