...
import { Plus, Search, X, RotateCcw, Eye, Trash2, Send, DollarSign, Star, TrendingUp } from "lucide-react";
...
export default function Sales({ ctx }) {
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
      <div>
        {isManager && <div style={{ padding: "8px 14px", borderRadius: 10, background: T.amberBg, border: `1px solid ${T.amber}30`, fontSize: 12, color: T.amber, fontWeight: 600 }}>⚠️ Manager mode — new sales require admin approval</div>}
      </div>
    ...
    <div className="glass" style={{ padding: "18px 18px 10px", borderRadius: T.radius }}>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={monthly}>
          ...
          <Bar dataKey="retAmt" name="Returns" fill={T.red} radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
    <div className="glass" style={{ padding: 18, borderRadius: T.radius }}>
      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        ...
        <tbody>{[...monthly].reverse().map((m, i) => <tr key={m.key} className="trow" style={{ fontWeight: i === 0 ? 700 : 400 }}>
          <td className="td" style={{ fontWeight: 600, color: i === 0 ? T.accent : T.text }}>{m.label}</td>
          <td className="td r">{m.orders}</td>
          ...
          <td className="td r" style={{ fontWeight: 600, color: T.text }}>{fmtCur(m.net)}</td>
          ...
          <td className="td r" style={{ color: m.net > 0 ? T.purple : T.textMuted }}>{m.net > 0 ? ((m.profit / m.net) * 100).toFixed(1) + "%" : "—"}</td>
        </tr>)}
          <tr style={{ borderTop: `2px solid ${T.accent}30` }}>
            <td className="td" style={{ fontWeight: 700, color: T.text }}>TOTAL (12M)</td>
            <td className="td r" style={{ fontWeight: 700 }}>{monthly.reduce((s, m) => s + m.orders, 0)}</td>
    ...
    <div className="glass" style={{ padding: 18, borderRadius: T.radius }}>
      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        <thead><tr>{["Bill No", "Date", "Channel", "Items", "Subtotal", "Discount", "Total", "", ""].map((h, i) => <th key={i} className="th" style={{ textAlign: ["Subtotal", "Discount", "Total"].includes(h) ? "right" : "left", width: h === "" ? 40 : "auto" }}>{h.toUpperCase()}</th>)}</tr></thead>
        <tbody>{saleBills.slice((pg - 1) * ps, pg * ps).map(b => {
          ...
          return <React.Fragment key={b.id}>
            <tr className="trow">
              ...
              <td className="td">{ch ? <span className="tag" style={{ background: ch.color + "18", color: ch.color }}>{ch.name}</span> : "—"}</td>
              <td className="td m">{(b.items || []).length} item{(b.items || []).length !== 1 ? "s" : ""}</td>
              <td className="td r">{fmtCur(b.subtotal)}</td>
              <td className="td r" style={{ color: b.discAmount > 0 ? T.red : T.textMuted }}>{b.discAmount > 0 ? `–${fmtCur(b.discAmount)}` : "—"}</td>
              <td className="td r" style={{ fontWeight: 700, color: T.text }}>{fmtCur(b.total)}</td>
            ...
            {exp[b.id] && <tr style={{ background: T.isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.015)" }}><td colSpan={9} style={{ padding: "14px 20px", borderBottom: `1px solid ${T.borderSubtle}` }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0, marginBottom: 12 }}>
                {(b.items || []).map((it, idx) => {
                  ...
                  return (
                    <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 40px 80px 80px 80px", gap: 8, alignItems: "start", padding: "7px 0", borderBottom: `1px solid ${T.borderSubtle}` }}>
                      <div>
                      ...
                      <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: T.textSub }}>{fmtCur(it.price)}</div><div style={{ fontSize: 9, color: T.textMuted }}>per unit</div></div>
                      <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: T.textMuted }}>{it.gstRate > 0 ? fmtCur(gstAmt) : "—"}</div><div style={{ fontSize: 9, color: T.textMuted }}>GST incl.</div></div>
                      <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, fontWeight: 700, color: T.text }}>{fmtCur(lineBase)}</div><div style={{ fontSize: 9, color: T.textMuted }}>line total</div></div>
                    </div>
                  );
                })}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, maxWidth: 260, marginLeft: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textSub }}><span>Subtotal (MRP)</span><span style={{ fontWeight: 600 }}>{fmtCur(b.subtotal)}</span></div>
                {(b.saleGstInfo || 0) > 0 && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textMuted }}><span>GST included in above</span><span>{fmtCur(b.saleGstInfo)}</span></div>}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, color: T.accent, borderTop: `1px solid ${T.borderSubtle}`, paddingTop: 6, marginTop: 2 }}><span>Net Total</span><span>{fmtCur(b.total)}</span></div>
              </div>
    ...
    <Modal open={retModal} onClose={() => setRetModal(false)} title="Record Sale Return" width={420}
      ...
      <div className="fgrid">
        ...
        <div className="s2" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: T.redBg, borderRadius: 10 }}>
          ...
          <label htmlFor="damret" style={{ fontSize: 13, fontWeight: 600, color: T.red, cursor: "pointer" }}>Mark Item as Damaged (Excluded from Stock)</label>
        </div>
      </div>
    </Modal>
  </div>;
