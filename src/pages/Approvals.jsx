...
import { Plus, Edit2, Trash2, ThumbsUp, ThumbsDown, CheckCircle, XCircle } from "lucide-react";
...
export default function Approvals({ ctx }) {
  ...
  const shown = tab === "pending" ? pending : hist;
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    {shown.length === 0 && <div className="glass" style={{ padding: "60px 20px", textAlign: "center", borderRadius: T.radius }}>
      ...
      <div style={{ fontWeight: 600, color: T.textSub, fontSize: 15 }}>No {tab === "pending" ? "pending approvals" : "history yet"}</div>
    </div>}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {req.proposedData.items.map((it, idx) => (
              <div key={idx} style={{ fontSize: 12, display: "flex", justifyContent: "space-between", color: T.text }}>
                <span>{it.productName} × {it.qty} {it.isDamaged ? <span style={{ color: T.red, fontWeight: 600, fontSize: 10, marginLeft: 6 }}>DAMAGED / EXCLUDED</span> : ""}</span>
      ...
      {req.action === "update" && req.currentData && req.proposedData && (
        <div className="fgrid" style={{ marginBottom: 12, gap: 10 }}>
          <div style={{ padding: "10px 12px", borderRadius: 10, background: T.isDark ? "rgba(248,113,113,0.07)" : "rgba(254,226,226,0.6)", border: `1px solid ${T.red}20` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.red, marginBottom: 6, letterSpacing: "0.05em" }}>CURRENT</div>
          ...
          <div style={{ padding: "10px 12px", borderRadius: 10, background: T.isDark ? "rgba(74,222,128,0.07)" : "rgba(220,252,231,0.6)", border: `1px solid ${T.green}20` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.green, marginBottom: 6, letterSpacing: "0.05em" }}>PROPOSED</div>
